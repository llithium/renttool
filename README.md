...
- `src/lib/` — city-aware estimated tax/budget math, formatting, search links, typed API client,
  `appState.svelte.ts` (the rent-plan workspace), `cityCatalog.svelte.ts` (canonical city identity,
  off-list placeholders, stored-city validation, and reference-safe enrichment),
  `rentLookupCoordinator.ts` (shared coordinate and rent-request cancellation),
  `rentPlanPresentation.svelte.ts` (the presentation seam used by both planning routes),
  `planRepresentation.ts` (canonical shareable URL encoding),
  `urlSync.svelte.ts` (state ⇄ address bar), and `salaryField.svelte.ts` (the salary input's own
  state). The workspace keeps plan orchestration, browser storage, and comparison-cap rules behind
  injected adapters; city identity and enrichment stay in the catalog, and lookup coordination stays
  behind the same public workspace seam.
